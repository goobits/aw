...
conflicting or interdependent mutations, and adaptive investigations where each
result may change the next step sequential. Do not split otherwise batchable
inspections across outer tool calls.
...
- `x-browser-handoff` (`.agents/skills/x-browser-handoff/SKILL.md`): self-contained, safe instructions for browser-only agents with structured return receipts.
